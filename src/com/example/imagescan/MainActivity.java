...
//				mGroupGridView.setAdapter(adapter);
				...
				if(list==null){ 
					Toast.makeText(getApplicationContext(), "没有找到图片可以上传,请拍照后上传", 0).show();
					finish();
					startActivity(new Intent(MainActivity.this, VideoActivity.class));
					return;
				}
				List<String> childList = mGruopMap.get(list.get(0).getFolderName());
				Intent mIntent = new Intent(MainActivity.this, ShowImageActivity.class);//直接进入目录
	...
	private void getImages() {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				File [] list = file.listFiles();//获取目录下所有文件
				for(int i = 0 ; i<list.length;i++){
					String path = list[i].getPath();		//获取图片的路径
